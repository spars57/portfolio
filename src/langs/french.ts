export const L_FRENCH = {
  landingPage: {
    title: 'Développeur Front-end React 👋🏻',
    description:
      'Bonjour ! Je suis Bruno Moisão, un développeur Frontend basé à Lisbonne, Portugal 📍',
    about: 'À propos',
    contact: 'Contacts',
    projects: 'Projets',
    aboutMe: 'À propos de moi',
    aboutText1:
      'Je suis un développeur spécialisé dans les applications Web. J’ai commencé ma carrière en 2018 à l’âge de 15 ans, où j’ai fait mes premiers pas dans le monde de la programmation.',
    aboutText2:
      "En 2022, à l'âge de 19 ans, j'ai conclu mon premier contrat en tant qu'ingénieur logiciel junior, me fournissant les conditions pour mettre en place ma première agence de médias sociaux et de marketing (SMMA).",
    aboutText3:
      "Aujourd'hui, j'aide les petites entreprises et les entrepreneurs à mondialiser leur entreprise grâce à des solutions de commerce électronique.",
  },
  languages: {
    main: '🌎 Langue',
    english: '🇬🇧 Anglais',
    portuguese: '🇵🇹 Portugais',
    german: '🇩🇪 Allemand',
    french: '🇫🇷 Français',
    russian: '🇷🇺 Russe',
    spanish: '🇪🇸 Espagnol',
    arab: '🇸🇦 Arabe',
    indian: '🇮🇳 Hindi',
  },
};
